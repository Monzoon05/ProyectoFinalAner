...
$sql_check = "SELECT id_queja FROM quejas WHERE id_multa_referencia = $id_multa AND estado = 'pendiente'";
...
$sql_insert = "INSERT INTO quejas (id_usuario_emisor, id_multa_referencia, motivo_queja, estado, fecha_queja) 
               VALUES ($id_jugador, $id_multa, '$motivo_seguro', 'pendiente', NOW())";

if (mysqli_query($conn, $sql_insert)) {
    header("Location: ../vista/perfiles/jugador.php?mensaje=Queja enviada correctamente. El administrador la revisara.");
    exit();
} else {
    header("Location: ../vista/perfiles/jugador.php?error=Error al guardar la queja: " . mysqli_error($conn));
    exit();
}
?>